  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Gestión de Usuarios</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Administra roles y permisos de usuarios</p>
      </motion.div>

      {usuarios.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
          <User size={64} className="mx-auto text-gray-300 dark:text-gray-600 mb-4" />
          <p className="text-gray-500 dark:text-gray-400 text-lg">No hay usuarios para mostrar</p>
        </motion.div>
      ) : (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-lg">
                ...
                <tr key={user.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-blue-200 dark:bg-blue-900 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-300 font-bold">
                        {user.nombre.charAt(0).toUpperCase()}
                      </div>
                      <span className="font-medium text-gray-900 dark:text-white">{user.nombre}</span>
                      {user.id === usuarioActual?.id && (
                        <span className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-1 rounded">Tú</span>
                      )}
                    </div>
                  </td>
                    ...
                    {editando?.id === user.id ? (
                      ...
                      <div className="flex items-center gap-2">
                        {user.rol === 'Administrador' ? (
                          ...
                          <span className="text-gray-600 dark:text-gray-400">Usuario estándar</span>
                        )}
                      </div>
                    )}
                  </td>
                      ...
                      {editando?.id === user.id ? (
                        <>
                          ...
                          <button
                            ...
                            className="bg-gray-300 dark:bg-gray-600 text-gray-800 dark:text-white px-3 py-1 rounded text-sm hover:bg-gray-400 dark:hover:bg-gray-500 transition"
                          >
                            Cancelar
                          </button>
                        </>
                        ...
                        <>
                          <button
                            ...
                            className="text-blue-500 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 disabled:text-gray-400 dark:disabled:text-gray-600"
                          >
                            <Edit2 size={18} />
                          </button>
                          <button
                            ...
                            disabled={user.id === usuarioActual?.id}
                            className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 disabled:text-gray-400 dark:disabled:text-gray-600"
                          >
                            <Trash2 size={18} />
                          </button>
                        </>
